...
### Phase 6の採用とPR #341更新（2026-07-16）

- ユーザー判断「Aで．PR #341を更新して」により折衷変種を採用
- PR用の直接計測（対fork main 5e8dd59，1T，インターリーブ5ラウンド）: heavy exec MoM 2.175→2.138s（**−1.7%**），total −1.4%＝**パリティ**（Phase 6記録の連鎖換算「パリティ〜+9%退行」を直接実測で訂正．ペアラウンド比中央値は−4.9%）
- 対Python再計測（同一セッション）: 1T 28.2x / 8T 49.7x（exec中央値．Python 59.3s / C++ 1T 2.102s / 8T 1.194s）．Phase 5の40.7xとの差は主にマシンドリフト（このマシンは熱で最大28%変動）とハーネス差
- 8T mainloop再確認: 0.359s（Phase 6の0.362sと1%以内で整合）
- PR #341: 559ee0bをcherry-pick（41363d9）してpush（5コミット構成），本文を折衷変種の設計（Python構造対応の明記）と新数値に更新．gh pr editはprojectCards deprecationエラーで失敗するため `gh api repos/.../pulls/341 -X PATCH` で更新した．更新後MERGEABLE
