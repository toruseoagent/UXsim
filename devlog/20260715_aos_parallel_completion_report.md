...
未送信（ユーザー指示待ち）。本ブランチはSoA版PR #340と同一ファイル群（traffi.h/traffi.cpp/bindings.cpp/wrapper/CMakeLists）を変更しており，**両PRは排他**（どちらを採用するかの判断が必要）。
送信時はコードコミット4個をorigin/mainベースのPRブランチにcherry-pick（devlog等フォーク専用ファイルは除外），ルートCMakeLists.txtの変更を含め，PR本文に上記ベンチ表を記載する。
