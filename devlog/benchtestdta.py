...
def run_due_dso(seed, cpp):
    ...
    results["dso_ttt"] = df_dso["total_travel_time"].sum()
    results["_dso_link_df"] = solver_DSO.W_sol.analyzer.link_to_pandas()
...
if __name__ == "__main__":
    ...
    # ============================================================
    # Scatter plots: Python vs C++ (seed=0)
    # ============================================================
    scatter_configs = [
        ("small", link_small, "_link_df"),
        ("large", link_large, "_link_df"),
    ...
    for j, (label, ld, key) in enumerate(scatter_configs):
        py_df = ld[seed0]["Python"][key]
        cpp_df = ld[seed0]["C++"][key]
        merged = py_df.merge(cpp_df, on="link", suffixes=("_py", "_cpp"))
        for i, metric in enumerate(metrics_plot):
            ax = axes[i, j]
            x = merged[f"{metric}_py"].values.astype(float)
            y = merged[f"{metric}_cpp"].values.astype(float)
    ...
    fig.suptitle(f"Link-level: Python vs C++ (seed={seed0})", fontsize=13)
    fig.tight_layout()
